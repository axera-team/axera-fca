import { ApiConstructorType as ApiConstructor } from "./api";
...
class APIRegistry {
  ...
  static #apis: Record<string, ApiConstructor> = Object.create(null);
   ...
   */
  static bulkLoadToRegistry(functionCollection: Record<string, ApiConstructor>): { loaded: number, failed: number } {
    let loadedCount = 0;
   ...
   */
  static add(name: string, handler: ApiConstructor): void;

  /**
   * Add multiple APIs to the registry via an object map
   */
  static add(name: Record<string, ApiConstructor>): void;
   ...
   * @returns
   */
  static add(name: string | Record<string, ApiConstructor>, handler?: ApiConstructor): void {
    if (this.#sealed) {
   ...
   * @returns
   */
  static get(name: string): ApiConstructor | null {
   ...
   */
  static expose(): Readonly<{ [name: string]: ApiConstructor }> {
  ...
  static unseal() {
    this.#sealed = false;
    const original = Object.getPrototypeOf(this.#apis) as Record<string, ApiConstructor>;
